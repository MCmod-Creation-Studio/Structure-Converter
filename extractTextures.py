...
def checkAndOutputTextures(metadata_name, model, modid, register_id, textures, textures_root):
    ...
    if model == 'cube' or model == "minecraft:cube":
        ...
        for old_name, new_name in mapping.items():
            ...
            shutil.copy(str(texture_path), os.path.join(exportFile, new_name))
    elif model == 'cube_all' or model == "minecraft:cube_all":
        texture_path = os.path.join(textures_root, textures['all'][len(modid + ':'):] + ".png")
        exportFile = modid + "\\" + register_id + "\\" + metadata_name
        os.makedirs(exportFile, exist_ok=True)
        shutil.copy(str(texture_path), os.path.join(exportFile, 'fill.png'))
        ...
        for i in ['left.png', 'right.png', 'front.png', 'back.png']:
            shutil.copy(str(sideTexture_path), os.path.join(exportFile, i))
        for i in ['bottom.png', 'top.png']:
            shutil.copy(str(endTexture_path), os.path.join(exportFile, i))
        ...
        for i in ['left.png', 'right.png', 'front.png', 'back.png']:
            shutil.copy(str(sideTexture_path), os.path.join(exportFile, i))
        shutil.copy(str(topTexture_path), os.path.join(exportFile, "top.png"))
        shutil.copy(str(bottomTexture_path), os.path.join(exportFile, "bottom.png"))
        pass
    else:
        raise RuntimeError(
            "无法处理：" + register_id + " " + metadata_name + "，因为存在未定义的特殊方块模型(" + str(model) + ")")
...
def process_blockstate(json_data, modid, register_id, textures_root):
    ...
    if not json_data.get('variants', {}) and not json_data.get('default', {}): raise RuntimeError(
        "遇到未定义的blockstate文件")
